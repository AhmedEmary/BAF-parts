...
class SaleOrder(models.Model):
    ...
    def _alzura_import_order(self, company, order_data):
        ...
        vals = {
            ...
            "partner_shipping_id": self._alzura_delivery_partner(
                partner,
                shipping,
    ...
    def _alzura_find_or_create_partner(self, buyer, country_code=False):
        ...
        vat_number = tax.get("sales_tax_identification_number")
        ...
        # If the buyer belongs to a cooperation, that company owns the buyer
        # contact and becomes the order's partner; otherwise the buyer is used.
        company = self._alzura_find_or_create_cooperation(
            buyer.get("cooperation") or {}
        )
        if company:
    ...
    def _alzura_find_or_create_cooperation(self, cooperation):
        """Get-or-create the buyer's cooperation as a company partner.

        Matched by cooperation number (ref 'ALZURA-COOP-<number>') then name.
        Returns an empty recordset when the buyer has no cooperation.
        """
        number = cooperation.get("number")
        name = cooperation.get("name")
        if not number and not name:
            return self.env["res.partner"].browse()

        Partner = self.env["res.partner"].sudo()
        ref = "ALZURA-COOP-%s" % number if number else False
        company = Partner.browse()
        if ref:
            company = Partner.search([("ref", "=", ref)], limit=1)
        ...
        return Partner.create(
            {
                ...
                "ref": ref,
                "is_company": True,
                "category_id": [(4, self._alzura_buyer_tag().id)],
            }
        )
    ...
    def _alzura_delivery_partner(self, parent, shipping, buyer, country_code=False):
        """Shipping address contact for the order.

        With an alternative address, a delivery child is found/created for it.
        Without one, the order ships to the parent (buyer/cooperation) only if
        the parent's stored address still matches the request; otherwise a
        delivery child carrying the request address is found/created instead.
        """
        delivery = shipping.get("delivery_address") or {}
        if delivery.get("use_alternative_address"):
            address = delivery.get("address") or {}
            contact = delivery.get("contact") or {}
        else:
            address = (buyer or {}).get("address") or {}
            contact = (buyer or {}).get("contact") or {}
            if self._alzura_address_matches(parent, address, country_code):
                return parent

        return self._alzura_get_or_create_delivery(
            parent, address, contact, country_code
        )

    def _alzura_address_matches(self, partner, address, country_code=False):
        """True when the partner's stored street/city/zip/country equal the
        request address (with country falling back to the order country code).
        """
        ...
        return (
            ...
            and (partner.zip or False) == (address.get("zip") or False)
            and partner.country_id.id == (country.id or False)
        )

    def _alzura_get_or_create_delivery(
        self, parent, address, contact, country_code=False
    ):
        """Find a delivery child of parent matching the address, else create it."""
        Partner = self.env["res.partner"].sudo()
        country_id = (
            self._alzura_country(address.get("country"))
            or self._alzura_country_by_code(country_code)
