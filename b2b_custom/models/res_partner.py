...
class ResPartner(models.Model):
    ...
    def baf_b2b_create_application(self, vals):
        ...
        if password and partner.email:
            # The application itself must survive even if portal-user
            # pre-provisioning fails (e.g. a stale archived login with the
            # same email, an unexpected constraint, etc.). The admin can
            # always re-create the user on approval.
            try:
                ...
                _logger.exception(
                    "BAF B2B: failed to pre-provision portal user for partner %s",
                    partner.id,
                )
        return partner
