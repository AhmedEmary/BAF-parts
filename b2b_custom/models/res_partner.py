...
class ResPartner(models.Model):
    ...
    def _baf_b2b_ensure_portal_user(self, password=None, active=True, add_b2b_group=True):
        ...
        # user_ids respects active_test=True by default, so an inactive
        # (pre-provisioned, awaiting approval) user is invisible here and the
        # branch below would create a duplicate, hitting the res_users_login_key
        # unique constraint. Bypass active_test so we find and reuse it.
        user = self.with_context(active_test=False).user_ids[:1]
        if not user:
