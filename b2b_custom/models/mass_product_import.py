    ...
    def _process_direct_sql(self, attachment_id):
        ...
        upsert_query = """
            INSERT INTO product_template (
                ...
                tracking, publish_date, base_unit_count,
                sale_ok, purchase_ok,
                baf_discount_code, baf_type_code, baf_mod, supplier_route,
                origin, hs_code, surcharge, weight, invoice_policy
            ) VALUES %s
        ...
        for row in rows:
            ...
            route = get_val(row, route_idx, 'eu_direct').lower() or 'eu_direct'
            if route not in ('de_table', 'lr_level', 'eu_direct'):
                route = 'eu_direct'
            ...
            data_batch_dict[default_code] = (
                ...
                'consu', True, uom_id, categ_id, True, 'no', 'none',
                fields.Datetime.now(), 0.0,
                True, True,
                baf_disc, baf_type, baf_mod, route,
                origin_id, hs_code, surcharge, weight, 'order',
            )
