...
class TestCartQtyLayout(HttpCase):
    ...
    def test_qty_update_keeps_current_page(self):
        self.authenticate('cql_user', 'cql_user')
        # 12 lines -> two pages of 10; page 2 holds the last 2 lines.
        products = self.env['product.template'].create([{
            'name': 'CQL Page Part %d' % i,
            'sku': 'CQL-PG-%03d' % i,
            'brand': self.brand.id,
            'list_price': 10.0 + i,
            'sale_ok': True,
            'is_published': True,
        } for i in range(12)]).product_variant_id
        ...
        order = self.env['sale.order'].search(
            [('partner_id', '=', self.customer.id), ('state', '=', 'draft')],
            order='id desc', limit=1)
        self.assertEqual(len(order.website_order_line), 12)

        page2_line = order.website_order_line[10]  # index 10 -> page 2
        result = self._post('/shop/cart/update', {
            'line_id': page2_line.id,
            'product_id': page2_line.product_id.id,
            'quantity': 4,
        })
        html = result.get('website_sale.cart_lines') or ''
        self.assertTrue(html, "no cart_lines fragment returned")
        # Only the current page's 2 lines come back, not all 12.
        self.assertEqual(
            html.count('class="o_cart_product '), 2,
            "AJAX re-render must return only the current page's lines")
        self.assertIn(
            'pagination', html,
            "AJAX re-render must include the pager")
