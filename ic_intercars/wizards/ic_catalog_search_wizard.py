...
and warehouse availability. From each row an admin can create a normal
Odoo product (flagged ``aftermarket``) with one click.
...
class IcCatalogSearchResult(models.TransientModel):
    ...
    def action_create_product(self):
        """Create (or refresh) a normal aftermarket product from this hit.

        Reuses the live price already fetched by the search; when the
        search ran without prices, one quote round-trip is made — a
        product must never be created with a zero cost.
        """
        self.ensure_one()
        backend = self.wizard_id.backend_id
        price_net = (self.customer_price_net
                     if self.has_price_data else
                     self.env['product.product']._baf_ic_live_cost(
                         backend, self.tow_kod))
        ic_data = {
            'sku': self.tow_kod,
            ...
            'genericArticleReferences': (
                [{'primary': True,
                  'genericArticleId': self.generic_article_id}]
                if self.generic_article_id else []
            ),
        }
        variant = self.env['product.product']._baf_find_or_create_ic(
            backend, ic_data, price_net=price_net,
        )
        return {
            'type': 'ir.actions.act_window',
            'name': _("Aftermarket Product"),
