"""IC identity fields + creation of IC parts as normal products.

IC parts are **normal catalog products**, flagged ``aftermarket`` and
filed under the *Aftermarket* product category. They carry a
manufacturer (Bosch, Valeo…) instead of a car-make brand. The helper
``_baf_find_or_create_ic()`` is what the cache list's *Create Products*
action and the live-search wizard call to get a stable Odoo product.
"""
...
class ProductTemplate(models.Model):
    ...
    # Aftermarket parts have no car-make brand (BAF's product.brand);
    # they have a manufacturer — Bosch, Valeo, Filtron…
    manufacturer_id = fields.Many2one(
        'product.manufacturer', string="Manufacturer", index=True,
        copy=False,
        help="Who actually makes the part. Independent of the car-make "
             "brand system used for OEM products.",
    )
    # Quality tier — the flag that separates IC aftermarket parts from
    # BAF's native OEM catalog.
...
class ProductProduct(models.Model):
    ...
    def _baf_find_or_create_ic(self, backend, ic_data, price_net=None):
        ...
        mfr_name = (ic_data.get('brand') or '').strip() or _("Unknown")
        Manufacturer = self.env['product.manufacturer'].sudo()
        manufacturer = Manufacturer.search(
            [('name', '=', mfr_name)], limit=1,
        ) or Manufacturer.create({'name': mfr_name})
        ...
        vals = {
            ...
            'sale_ok': True,
            'purchase_ok': True,
            # BAF convention: `sku` carries the raw part number. IC
            # products have no car-make brand, so b2b_custom's
            # brand-prefix compute can't produce a reference — we set
            # an explicit 'IC_<sku>' one (unique because tow_kod is).
            'sku': ic_sku,
            'default_code': f"IC_{ic_sku}",
            'ic_sku': ic_sku,
        ...
        categ = self.env.ref('ic_intercars.product_categ_aftermarket',
                             raise_if_not_found=False)
        if categ:
        ...
        _logger.info(
            "Created IC product sku=%s manufacturer=%s odoo_id=%s",
            ic_sku, mfr_name, variant.id,
        )
        return variant
