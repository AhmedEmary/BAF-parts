...
class ProductTemplateBafPricing(models.Model):
    ...
    def _baf_alternative_direct_vendors(self, default_price, partner=None):
        ...
        Task #51 — when the customer configured a max delivery cap
        (`res.partner.baf_max_delivery_weeks`), vendors whose delivery frame
        exceeds it are neither displayed nor considered.
...
class ProductProductBafPricing(models.Model):
    ...
    def baf_get_best_vendor(self, customer=None):
        ...
        Default ranking: shortest delivery period first, then lowest price,
        then vendor id (ascending). A vendor with no delivery period set
        (0/empty) is ranked last (slowest).

        Task #51 — when `customer` is passed and has a delivery cap set
        (`res.partner.baf_max_delivery_weeks`), the picker:
          - drops every candidate whose delivery frame exceeds the cap;
          - ranks the survivors by **cheapest first**, then delivery, then
            vendor id (mirrors the spec's "cheapest supplier within the
            allowed delivery time"). Vendors with no delivery frame still
            rank last inside the window (rank 9999).
        ...
        if cap:
            # Task #51: cheapest within the allowed window; ties broken by
            # delivery, then lowest id.
            priced.sort(key=lambda c: (round(c['price'], 4),
                                       baf_delivery_rank(c['delivery_weeks']),
                                       c['vendor'].id))
        else:
            # No cap → keep the historical ordering (shortest delivery wins,
            # ties break by price, then id). Preserves behaviour for customers
            # that don't opt into the window.
