...
# Identity header stamped on every /mcp response (including CORS preflights).
# Kept alongside the REST equivalent in ``response_utils`` so a fork can change
# both surfaces from one place if it ever moves to a shared constants module.
...
class MCPDispatcher(http.Dispatcher):
    ...
    def pre_dispatch(self, rule, args):
        ...
        # Identity header on every /mcp response (real request and CORS
        # preflight): lets fronting infrastructure identify an AgenticBrains
        # build without inspecting the body.
        headers.set(AB_IDENTITY_HEADER, AB_IDENTITY_VALUE)
        if allowed_origins:
