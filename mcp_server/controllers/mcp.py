...
SERVER_NAME = "agenticbrains-mcp-server"
# serverInfo.description (2025-11-25): human-readable context clients may show.
SERVER_DESCRIPTION = (
    "AgenticBrains MCP Server for Odoo: read and write Odoo records with "
    "per-model access control, audit logging and rate limiting."
)
...
class MCPController(http.Controller):
    ...
    @staticmethod
    def _negotiate_protocol_version(requested):
        """Pick the protocol revision echoed back to the client.

        Honour the requested revision when this server advertises it; fall
        back to the preferred one otherwise. Kept as a separate method so a
        fork can plug in its own negotiation policy (feature-flag on
        newer revisions, staged rollouts, ...) without touching the rest of
    ...
    def _build_server_info():
        """Return the ``serverInfo`` object advertised by the initialize handshake.

        Extracted from :meth:`_initialize` so the branding surface (name,
        description, version, plus AgenticBrains-specific hints under
        ``metadata``) sits in one place. Clients that ignore the block are
        unaffected; those that display it get a richer identity card.
        """
        return {
            "name": SERVER_NAME,
            "version": utils.get_mcp_server_version(),
            "description": SERVER_DESCRIPTION,
            # ``metadata`` is a free-form extension slot for MCP servers.
            # Publishing vendor/product/build here lets a client's server
            # panel render a proper "About" section for AgenticBrains without
            # us claiming any spec field we don't own.
            "metadata": {
                "vendor": "AgenticBrains",
                "product": "AgenticBrains MCP Server for Odoo",
                "homepage": "https://agenticbrain.tech/",
            },
        }
    ...
    def _initialize(self, params):
        ...
        result = {
            ...
            "serverInfo": self._build_server_info(),
        }
