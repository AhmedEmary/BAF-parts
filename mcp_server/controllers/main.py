...
"""AgenticBrains MCP - REST controller surface.

Hosts the small REST endpoints that predate the native ``/mcp`` JSON-RPC route
and are still used by the legacy bridge (``uvx mcp-server-odoo``) and simple
integration checks: ``/mcp/health`` (public liveness probe), ``/mcp/system/info``,
``/mcp/auth/validate``, ``/mcp/models*``.
"""
...
# Advertised alongside the raw module version in ``/mcp/health`` so operators
# can tell an AgenticBrains-branded deployment apart at a glance. Bump when a
# fork wants its own identity in the health payload.
AB_PRODUCT_NAME = "AgenticBrains MCP Server for Odoo"
AB_VENDOR = "AgenticBrains"
...
class McpAPIController(http.Controller):
    ...
    def health_check(self, **kwargs):
        """Public liveness probe. Also identifies the AgenticBrains build.

        Extended over the historical shape with a few identity fields so
        monitoring, uptime checks and fleet dashboards can distinguish this
        AgenticBrains-branded fork from other MCP servers on the same host.
        The ``status``/``mcp_server_version`` keys are preserved verbatim for
        backwards compatibility with existing probes.
        """
        mcp_server_version = utils.get_mcp_server_version()
        data = {
            "status": "ok",
            "mcp_server_version": mcp_server_version,
            "product": AB_PRODUCT_NAME,
            "vendor": AB_VENDOR,
