...
@tagged("agenticbrains_unit", "post_install", "-at_install")
class TestMcpSecurity(common.TransactionCase):
    ...
    def test_settings_menu_access(self):
        ...
        # Look up the branded root by its XML ID first; fall back to a name
        # search that tolerates both the new ("AgenticBrains MCP") and legacy
        # ("MCP") labels so the assertion doesn't depend on the current copy.
        try:
            menu_root = self.env.ref("mcp_server.ab_mcp_menu_root")
        except ValueError:
            menu_root = self.env["ir.ui.menu"].search(
                [("name", "in", ("AgenticBrains MCP", "MCP"))], limit=1
            )

        if not menu_root:
            self.skipTest("AgenticBrains MCP menu not found, skipping test")
